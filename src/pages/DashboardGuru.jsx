          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="date" name="tarikh_mula" required value={form.tarikh_mula} onChange={handleChange}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Tarikh Tamat</label>
                <input type="date" name="tarikh_tamat" required value={form.tarikh_tamat} onChange={handleChange}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="time" name="masa_mula" required value={form.masa_mula} onChange={handleChange}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Masa Tamat (Waktu Ganti)</label>
                <input type="time" name="masa_tamat" required value={form.masa_tamat} onChange={handleChange}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
            ...
            <div>
              ...
              <input type="text" name="kelas" required value={form.kelas} onChange={handleChange}
            ...
            {ralat && <p className="text-red-500 text-sm bg-red-50 border border-red-200 rounded-lg px-3 py-2">{ralat}</p>}
            {berjaya && <p className="text-green-600 text-sm bg-green-50 border border-green-200 rounded-lg px-3 py-2">Permohonan berjaya dihantar!</p>}
                ...
                <div key={item.id} className="border border-slate-200 rounded-xl p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">{item.masa_mula} – {item.masa_tamat} · {item.kelas}</p>
